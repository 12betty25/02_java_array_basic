...
public class ArrayEx18_풀이 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.println("[" + turn1 +"바퀴]");
			
			for (int i = 0; i < p2.length; i++) {
				System.out.print(p2[i] + " ");
			}
			System.out.println("[" + turn2 +"바퀴]");
			System.out.println();
			...
			if (turn % 2 == 0) {
				...
				idx1 %= 8;		//한바퀴를 돌고나서의 나머지
				
				p1[idx1] = 1; 	//나머지번째에 player 1 넣어주기

			}
			
			else if (turn % 2 == 1) {
				int move = ran.nextInt(3)+1;
				System.out.println("[p2컴퓨터] : " + move);
				System.out.println();
				
				p2[idx2] = 0;	//현재 플레이어2의 위치를 0으로 바꿔준다
				idx2 += move;
			...
			if (idx1 == idx2 && idx1!=0 && turn % 2 == 0) {
				...
				p2[idx2] = 0;	//현재 위치의 숫자를 0
				idx2 = 0;		//p2 위치 초기화 
				p2[idx2] = 2;	//초기화된 위치에 p2 놓아주
			}
